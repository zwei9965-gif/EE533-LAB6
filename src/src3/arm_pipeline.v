// ============================================================
// arm_pipeline.v  ?  ??? 5-Stage ARM Pipeline (Part 2)
// EE 533 Lab 6  Team 4
//
// ? Step 4 ???????????????????
//   - 4 ??? PC???????
//   - 4 ? CPSR?????? N/Z/C/V?
//   - ??????????IF?ID?EX?MEM?WB
//   - ??????? 64 ??4×16??? {thread_id, reg_addr} ??
//
// ??????
//   4 ??????? IF ????4????
//   ??????????????????4???
//   ? ????? WB ??????????? ID
//   ? ??? RAW hazard??? forwarding ? load-use stall
//
// ??????
//   ??? EX ????IF/ID ?????????????????
//   ???? pc[idex_thread] = branch_target
//   2??????? IF ??????????
//
...
module arm_pipeline (
    ...
    // ???? + 4 ? PC
    // =========================================================
    reg [1:0] if_thread;      // ?? IF ???????? (0?1?2?3?0?...)
    reg [8:0] pc [0:3];       // ????? PC?????

    // branch_taken / branch_target_pc ? EX ??????????
    wire        branch_taken;
    wire [8:0]  branch_target_pc;

    // ?? ??????? ISim ?? wire ?? ??????????????????
    // ?? reg ??? always ???????????????????
    reg  [1:0]  idex_thread;       // EX ??????PC ????
    reg [31:0]  exmem_alu_result;  // EX/MEM ???imem data_addr ??
    reg  [1:0]  memwb_thread;      // WB ??????regfile wb_thread ??

    // ????? PC?word ???
    // T0->imem[0], T1->imem[32], T2->imem[64], T3->imem[96]
    // ??? imem ? 48 words?GCC -O0 ??? 32-37 words?????
    // T0: word[0..47]  T1: word[48..95]  T2: word[96..143]  T3: word[144..191]
    ...
    always @(posedge clk) begin
        if (rst) begin
            ifid_instr  <= 32'hE1A0B00B; // NOP (MOV R11,R11)
            ifid_pc     <= 9'b0;
            ifid_thread <= 2'b0;
        end else begin
            ifid_instr  <= if_instr;
            ifid_pc     <= pc[if_thread]; // NBA??? pc[if_thread]?fetch ???
    ...
    arm_regfile regfile_inst (
        .clk       (clk),
        .rst       (rst),
        .thread_id (ifid_thread),   // ID ????????????
        .wb_thread (memwb_thread),  // WB ????????????
    ...
    // WB ??? wb_thread = memwb_thread????? regfile?
    // =========================================================
